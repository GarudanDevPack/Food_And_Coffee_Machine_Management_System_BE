...
export class UsersDocumentRepository implements UserRepository {
  ...
  async findAll(): Promise<User[]> {
    const userObjects = await this.usersModel.find().sort({ createdAt: -1 });
    return userObjects.map((u) => UserMapper.toDomain(u));
  }
  ...
  async findByCustomerId(customerId: string): Promise<NullableType<User>> {
    if (!customerId) return null;
    const userObject = await this.usersModel.findOne({ customerId });
    return userObject ? UserMapper.toDomain(userObject) : null;
  }
